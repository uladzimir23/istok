...
| ADR-011 | Custom Admin SPA over PocketBase | proposed |
...
> из ADR-005 («редактор фабрики без git»). ADR-011 — кастомная SPA-админка поверх PB
> для нетехнического редактора фабрики. Модель — static export + rebuild-webhook
...
5. **Портфолио госзаказа** — страница `/proekty` готова (6 проектов в `content/projects/`,
   content-as-code). Контент **иллюстративный** — заменить на реальные кейсы от клиента.
...
8. **Визуальный QA брейкпоинтов** — пороги сместились ≤80px после миграции на токен-скейл
   (ревью DS 2026-06-27); проверить вёрстку на границах sm/md/lg/xl.
